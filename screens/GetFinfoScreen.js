...
const GetFinfoScreen = ({ navigation, route }) => {
  ...
  const handleSaveData = async () => {
    try {
      ...
      const accountDocRef = await addAccountToFirestore(hashedAccountNumber, hashedAccountPassword);
      const cardDocRef = await addCardToFirestore(hashedCardNumber, hashedCardPassword);
  ...
  const addAccountToFirestore = async (hashedAccountNumber, hashedAccountPassword) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.titleContainer}>
        ...
        <Text style={styles.descriptionText}>카드, 계좌 정보 입력이 필요합니다.</Text>
      </View>
      <View style={styles.inputContainer}>
        <TextInput style={styles.input} placeholder="계좌번호" value={accountNumber} onChangeText={setAccountNumber} />
        ...
        <TextInput style={styles.input} placeholder="카드번호" value={cardNumber} onChangeText={setCardNumber} />
